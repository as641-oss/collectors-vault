...
test.describe('Task 9 - Notification Center (Private)', () => {
  test('seller receives notification after buyer places order', async ({ browser }) => {
    ...
    await buyerPage.getByPlaceholder('Email').fill('buyer1@vault-private.dev');
    await buyerPage.getByPlaceholder('Password').fill('VaultPass123!');
    ...
    await buyerPage.locator('input[name="phone"]').fill('5557654321');
    ...
    await buyerPage.locator('input[name="line1"]').fill('456 Private Street');
    ...
    await buyerPage.getByPlaceholder(/search/i).fill('Private Sample Comics Item 7');
    await expect(buyerPage.locator('body')).toContainText('Private Sample Comics Item 7');

    await buyerPage.goto('http://localhost/marketplace/sample-comics-7-private');
    ...
    await sellerPage.getByPlaceholder('Email').fill('seller1@vault-private.dev');
    await sellerPage.getByPlaceholder('Password').fill('VaultPass123!');

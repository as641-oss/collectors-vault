...
test.describe('Task 7 - Admin User Management (Private)', () => {
  test('admin can view users, change role, and toggle user status', async ({ page }) => {
    await page.goto('http://localhost/login');

    await page.getByPlaceholder('Email').fill('admin@vault-private.dev');
    ...
    await expect(page.locator('body')).toContainText('buyer1@vault-private.dev');

    const buyerRow = page.locator('tr', {
      has: page.locator('td', { hasText: 'buyer1@vault-private.dev' })
    });

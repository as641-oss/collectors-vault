...
test.describe('Task 1 - Marketplace Search', () => {
  test('search filters listings and shows empty state', async ({ page }) => {
    await page.goto('http://localhost');

    await expect(page.getByTestId('search-input')).toBeVisible();

    await page.getByTestId('search-input').fill('Trading');
    await page.getByRole('button', { name: 'Search' }).click();

    await expect(page.locator('[data-testid="listing-card-link"]').first()).toContainText('Trading');

    await page.getByTestId('search-input').fill('zzzz-not-found');
    await page.getByRole('button', { name: 'Search' }).click();

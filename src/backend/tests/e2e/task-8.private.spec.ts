...
test.describe('Task 8 - Bundle Checkout (Private)', () => {
  test('buyer can add multiple seeded listings to cart and checkout', async ({ page }) => {
    ...
    await page.locator('input[name="phone"]').fill('5557654321');
    await page.getByRole('button', { name: /save profile/i }).click();

    await page.getByRole('button', { name: /add address/i }).click();

    await page.locator('input[name="fullName"]').fill('Nina Buyer');
    await page.locator('input[name="line1"]').fill('456 Private Street');
    ...
    await page.goto('http://localhost/marketplace/sample-comics-7-private');
    await page.getByRole('button', { name: 'Add to Cart' }).click();

    await page.goto('http://localhost/marketplace/sample-action-figures-8-private');
    await page.getByRole('button', { name: 'Add to Cart' }).click();

    await page.goto('http://localhost/cart');
    await expect(page.locator('body')).toContainText('Private Sample Comics Item 7');
    await expect(page.locator('body')).toContainText('Private Sample Action Figures Item 8');
